...
// Fetch all mobile assets from the database
$result = mysqli_query($conn, "SELECT * FROM `asset` WHERE `asset_type` = 'Laptop'");

if (!$result) {
    echo "Error fetching mobile assets: " . mysqli_error($conn);
}

// Count the number of records using prepared statements
$count_sql = "SELECT COUNT(*) AS record_count FROM asset WHERE `asset_type` = 'Laptop'";
...
<body>
    ...
    <div class="flex-1 p-8">
        <div class="flex items-center justify-between"> <!-- Flex container for layout -->
            ...
            <h5 class="text-2xl uppercase font-medium">Laptop</h5>
                ...
                <?php if (mysqli_num_rows($result) > 0) : ?>
                    <?php while ($row = mysqli_fetch_assoc($result)) : ?>
                        <tr class="bg-gray-50 border-b dark:bg-gray-800 dark:border-gray-700 font-bold">
                            <td class="text-center px-6 py-4"><?php echo htmlspecialchars($row['asset_id']); ?></td>
                            <td class="text-center px-6 py-4"><?php echo htmlspecialchars($row['asset_type']); ?></td>
                            <td class="text-center px-6 py-4"><?php echo htmlspecialchars($row['serial_no']); ?></td>
                            <td class="text-center px-6 py-4"><?php echo htmlspecialchars($row['model_name']); ?></td>
                            <td class="text-center px-6 py-4"><?php echo htmlspecialchars($row['deployed_to']); ?></td>
                            <td class="text-center px-6 py-4">
                                <div class="flex items-center space-x-2 justify-center">
                                    <a title="view" href="laptop_view.php?id=<?php echo $row['id']; ?>" class="text-black ">
                                    ...
                                    <form method="POST" action="laptop_delete.php" style="display:inline;">
                                    ...
                                    <a title="archive" href="asset_archive.php?id=<?php echo $row['id']; ?>" class="text-black ">
